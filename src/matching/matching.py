...
class Matching :
    ...
    def __match(self,set_news : list[dict], suppliers : dict) -> list[dict] :
        """
        Matches companies mentioned in the news dataset with the list of suppliers. Each suplier have a city and a company.
        We have three levels of matching:
        1. The first one is matching only the supplier name and the extracted company name
        2. The second match concern the case we have match both the company name but also the contry of the supplier. e.g the extracted company name and the supplier have boath the same name and are located in the same country.
        3. The last match concern the case where the name, the contry and the city of the supplier and the extracted company name are matched. 
