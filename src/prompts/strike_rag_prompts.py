...
from src.output_parsers.output_parsers import AutomotiveSector, BusinessSectors, Company, Temporality, LaborStrike
...
prompt1 = PromptTemplate(
    ...
    For instance, if you know that a particular company goes by its full legal name, use that instead of abbreviations or nicknames. If there isn't a specific company mentioned in the context that is undergoing a strike, your response should be 'None'. 
        Additionally, please provide the city and country where the strike is occurring, taking place, or expected to happen.The country must be translate into english, for exemple if the detected country are 'France' it must be translate into english spelling to be 'French' and not be let as 'France'.
