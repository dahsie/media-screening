...
def groupByName(json_data : list[dict])-> list[dict]:
    """
    Groups articles by impacted company, core company names and countries, identifying duplicates and associating them as sub-articles.
    ...
    for i in  range(len(json_data) -1):
        ...
        if len(json_data[i]['impacted_company']) != 0:
            ...
            countries = []
            for location in json_data[i]['locations']: # countries extraction
            ...
            for j  in range(i+1, len(json_data)):
                ...
                countries1 = []
                for location in json_data[j]['locations']: # countries extraction
                    if location['country'] not in countries1:
                        countries1.append(location['country'])
                ...
                if len(json_data[j]['impacted_company']) != 0 :
                    ...
                    if (name1.lower().strip() == name.lower().strip() or core_name1.lower().strip() == core_name.lower().strip()) and json_data[j]['duplicated'] == "no" and relevant == relevant1 and countries == countries1:
        ...
        if item['relevant'] == 'yes':
            for index2, location in enumerate(item['locations']):
                if isinstance(location, dict) and location['city'] != '':
                    try: # One can have an exception due to the fact that que can not find geographical coordonate for a given city
                        loc = geolocator.geocode(location['city'])
                        if loc is None or loc == '':
                            json_data[index]['locations'][index2]['latitude'] = ''
                            json_data[index]['locations'][index2]['longitude'] = ''
                        else:
