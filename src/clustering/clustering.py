...
class Clustering:
    ...
    def __init__(self, percentile: float = 80, linkage = 'ward', metric = 'euclidean'):
        """
        Initializes the Clustering class.
    ...
    def distance_threshold(self) -> None:
        ...
        """
        if self.is_fitted:
            sil_argmax, sil_max = np.argmax(self.sil_scores), max(self.sil_scores)
            silhouettes = self.sil_scores[:sil_argmax +1]
            distances = self.dist[:sil_argmax +1]
            sil_threshold, index = 0., sil_argmax
            if self.__percentile > 1 and self.__percentile < 100:
                sil_threshold = (self.__percentile/100)* sil_max
                for i in range(len(silhouettes) -1):
                    if silhouettes[i] <= sil_threshold and silhouettes[i+1] >= sil_threshold:
                        index = i
                        break
                self.threshold = distances[index]
            else:
                raise ValueError(f"Percentile should be between 0 and 100 but receive {self.__percentile}")
        else:
    ...
    def predict(self, xtrain: np.ndarray) -> np.ndarray:
        ...
        self.__model.distance_threshold = self.threshold
